...
from uad.layers import DFConv2d, IOULoss, NaiveGroupNorm
from .gaussian_fcos_outputs import GaussianFCOSOutputs

__all__ = ["GAUSSIAN_FCOS"]
...
class GAUSSIAN_FCOS(nn.Module):
    ...
    def __init__(self, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        self.gaussian_sigma_weight = cfg.MODEL.FCOS.SIGMA_LOSS_WEIGHT
        self.atss_on              = cfg.MODEL.FCOS.ATSS_ON #ywlee
        self.atss_topk            = cfg.MODEL.FCOS.ATSS_TOPK #ywlee
        self.atss_anchor_sizes    = cfg.MODEL.FCOS.ATSS_ANCHOR_SIZES #ywlee
        # fmt: on
        ...
        self.fcos_head = GaussianFCOSHead(cfg, [input_shape[f] for f in self.in_features])
    ...
    def forward_head(self, features, top_module=None):
        ...
        return pred_class_logits, pred_deltas, top_feats, bbox_towers
    ...
    def forward(self, images, features, gt_instances=None, top_module=None):
        ...
        logits_pred, reg_pred, sigma_pred, top_feats, bbox_towers = self.fcos_head(
            features, top_module, self.yield_proposal)
        ...
        outputs = GaussianFCOSOutputs(
            images,
            locations,
            logits_pred,
            reg_pred,
            sigma_pred,
            self.gaussian_sigma_weight,
            ...
            gt_instances,
            atss_topk=self.atss_topk if self.atss_on else None,
            atss_anchor_sizes=self.atss_anchor_sizes if self.atss_on else None
        )
...
class GaussianFCOSHead(nn.Module):
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        ...
        self.sigma_pred = nn.Conv2d(
            in_channels, 4, kernel_size=3,
            stride=1, padding=1
        )
        self.certainty_rep = nn.Sequential(
            nn.Conv2d(4, 4, 1),
        ...
        for modules in [
            ...
            self.bbox_pred,  self.sigma_pred,
            self.certainty_rep  # [new]
        ]:
    ...
    def forward(self, x, top_module=None, yield_bbox_towers=False):
        ...
        for l, feature in enumerate(x):
            ...
            sigma_pred = self.sigma_pred(bbox_tower).sigmoid()
            sigma.append(sigma_pred)

            certainty_rep = self.certainty_rep(1 - sigma_pred.detach())
            logit = self.cls_logits(cls_tower * certainty_rep)
            logits.append(logit)
        ...
        return logits, bbox_reg,  sigma, top_feats, bbox_towers