...
def _create_text_labels(classes, scores, class_names, certainty=None):
    """
    Args:
    ...
    if scores is not None:
        if labels is None:
                if certainty is None:
                    ...
                    labels = ["{:.0f}%, C:{:.0f}%".format(s * 100, c * 100) for s, c in zip(scores, certainty)]
        else:
                if certainty is None:
                    labels = ["{} {:.0f}%".format(l, s * 100) for l, s in zip(labels, scores)]
                else:
                    labels = ["{} {:.0f}%, C:{:.0f}%".format(l, s * 100, c * 100) for l, s, c in zip(labels, scores, certainty)]

    return labels


def _create_text_certainties(certainties):
    """
    Args:
        classes (list[int] or None):
        scores (list[float] or None):
    ...
    """
    #l, t, r, b
    certainties = certainties.permute(1, 0)
    print(certainties.shape)
    labels = [0 for _ in range(4)]
    if certainties is not None:
        labels[0] = ["C_L:{:.0f}%".format(c * 100) for c in certainties[0]]
        labels[1] = ["C_T:{:.0f}%".format(c * 100) for c in certainties[1]]
        labels[2] = ["C_R:{:.0f}%".format(c * 100) for c in certainties[2]]
        labels[3] = ["C_B:{:.0f}%".format(c * 100) for c in certainties[3]]
...
class Visualizer:
    ...
    def draw_instance_predictions(self, predictions):
        ...
        certainty = predictions.certainty if predictions.has("certainty") else None
        certainties = predictions.certainties if predictions.has("certainties") else None
        classes = predictions.pred_classes if predictions.has("pred_classes") else None
        labels = _create_text_labels(classes, scores, self.metadata.get("thing_classes", None), certainty)
        keypoints = predictions.pred_keypoints if predictions.has("pred_keypoints") else None
        #uncertainties
        certainties_labels = None
        if certainties is not None:
            print(certainties.shape)
            certainties_labels = _create_text_certainties(certainties)
        ...
        if self._instance_mode == ColorMode.SEGMENTATION and self.metadata.get("thing_colors"):
            ...
            alpha = 0.8
        elif self._instance_mode == ColorMode.LINEAR:
            COL = MplColorHelper('jet', 0.01, 1.0)
            values = certainty if certainty is not None else scores
            colors = [COL.get_rgb(v) for v in values]
            alpha = 0.3
        else:
            colors = None
            alpha = 0.5
        ...
        self.overlay_instances(
            ...
            alpha=alpha,
            certainties=certainties_labels
        )
        return self.output
        ...
        *,
        ...
        for i in range(num_instances):
            ...
            if labels is not None:
                ...
                lighter_color = self._change_color_brightness(color, brightness_factor=0.5)
                font_size = (
                ...
                #
                # text_pos = (int((x0+x1)/2), int((y0+y1)/2))
                # text_pos = (int((x0+x1)/2), y0)
                self.draw_text(
                    labels[i],
                    text_pos,
                    color=lighter_color,
                ...
                if certainties is not None:
                    ...
                    # adjust text position for better visualization
                    text_pos_list = [(x0-10, int((y0+y1)/2)), (int((x0+x1)/2)+20, y0-20), (x1-10, int((y0+y1)/2)), (int((x0+x1)/2), y1)]
